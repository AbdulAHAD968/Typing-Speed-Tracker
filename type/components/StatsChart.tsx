...
export default function Stats() {
  ...
  const [visibleCount, setVisibleCount] = useState(5); // show 5 tests initially
  ...
  if (loading) {
    return (
      <div className="text-center text-coffee-700 text-lg py-8">Loading...</div>
    );
  }

  return (
    <div className="max-w-5xl mx-auto p-8 bg-peach-50 rounded-lg shadow-md border border-peach-200">
      <h1 className="text-3xl font-bold mt-8 mb-10 text-center text-brown-800">
      ...
      {results.length === 0 ? (
        ...
        <div>
          ...
          <div className="mb-8">
            <h2 className="text-xl font-semibold mb-4 text-coffee-700">
              Progress Overview
            </h2>
              ...
              <table className="min-w-full bg-peach-50 border border-peach-200">
                ...
                <tbody>
                  {visibleResults.map((result) => (
                    <tr
                      key={result._id}
                      className="odd:bg-white even:bg-peach-100/60"
                    >
            ...
            {visibleCount < results.length && (
              <div className="flex justify-center gap-4 mt-4">
                <button
                  ...
                  className="px-4 py-2 bg-peach-400 hover:bg-peach-600 text-white rounded-lg shadow-md"
                >
                  Load More
                </button>
                <button
                  onClick={handleShowAll}
                  className="px-4 py-2 bg-coffee-600 hover:bg-coffee-800 text-peach-100 rounded-lg shadow-md"
                >
                  Show All
                </button>
              </div>
